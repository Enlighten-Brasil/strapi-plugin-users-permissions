...
module.exports = {
  ...
  async updatePermissions() {
    ...
    if (!_.isEqual(permissionsFoundInDB.sort(), permissionsFoundInFiles.sort())) {
      ...
      if (permissionDropStrategy === 'delete') {
        ...
        console.log(' ❗ Removing permissions:', toRemove.length)
        await Promise.all(
          toRemove.map(permission => {
            const { type, controller, action, roleId: role } = permission;
            console.log(`🚫 Removed permission: Role: ${role} Controller: ${controller} Action: ${action} Type: ${type} `)
            return query.delete({ type, controller, action, role });
          })
        );
      } else if (permissionDropStrategy === 'skip') {
        console.log('🏃 Skipping permission removal')
      }
    }
  },
  async initialize() {
